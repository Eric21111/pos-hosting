...
const EditBrandPartnerModal = ({ isOpen, onClose, onSuccess, brandPartner }) => {
  ...
  useEffect(() => {
    if (isOpen && brandPartner) {
      setFormData({
        brandName: brandPartner.brandName || '',
        email: brandPartner.email || '',
        contactNumber: brandPartner.contactNumber || '',
        contactPerson: brandPartner.contactPerson || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.message || 'Failed to update brand partner.');
      }

      onSuccess?.(data.data);
      onClose();
    } catch (err) {
      console.error('Failed to update brand partner:', err);
      setError(err.message || 'Failed to update brand partner.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[10005] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          ...
          aria-label="Close edit brand partner modal">
                ...
                <form onSubmit={handleSubmit} className="p-8 pt-10 space-y-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Edit Brand Partner</h2>
                            <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Update brand partner profile</p>
                        </div>
                    </div>

                    <div
            className={`w-36 h-36 mx-auto rounded-full flex items-center justify-center cursor-pointer border border-dashed ${theme === 'dark' ?
            'bg-gradient-to-b from-[#2A2724] to-[#1E1B18] border-gray-600 hover:border-[#AD7F65]' :
            'bg-gradient-to-b from-gray-50 to-gray-200 border-gray-300 hover:border-[#AD7F65]'}`
            }
...
export default EditBrandPartnerModal;